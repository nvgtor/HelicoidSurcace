...
public class Spring
{
	...
	public Spring(SpringSurfaceView mv,
				  ...
				  int nCol ,int nRow)
	{
		...
		new Thread() {
			public void run() {
				while (true) {
					yAngle += 2 * Constant.TOUCH_SCALE_FACTOR;
					try {
						Thread.sleep(50);
					} catch (Exception e) {
						e.printStackTrace();
	...
	public void initVertexData(
		...
		average = nCol * nRow/ Constant.picture_Number * 6;
	...
	public void initShader(SpringSurfaceView mv)
	{
		//加载顶点着色器的脚本内容
...
//        调用glGetAttribLocation得到变量aPosition和aTexPosition的句柄
		//获取程序中顶点位置属性引用id
...
//    	 传递GL_BLEND给glDisable方法，在渲染过程中禁用颜色的混合
		GLES20.glDisable(GLES20.GL_BLEND);
		//将最终变换矩阵传入shader程序
		GLES20.glUniformMatrix4fv(muMVPMatrixHandle, 1, false, MatrixState.getFinalMatrix(), 0);
//         使用glVertexAttribPointer连接aPosition和aTexPosition的句柄到各自的verticesBuffer和textureBuffer
		//传送顶点位置数据
...
//         使用glBindTexture方法绑定纹理（作为draw方法的参数传入）到片段着色器上
		for (int i = 0; i < Constant.picture_Number; i++) {
			GLES20.glBindTexture(GL10.GL_TEXTURE_2D,Constant.pictures[i]);// 绑定纹理id
//        	 GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT);
			GLES20.glDrawArrays(GLES20.GL_TRIANGLES, average * i, average);// 三角形绘制
		}
